...
INTERVALOS = [None, 1, 7, 30]


def calcular_proxima_revisao(
    total_revisoes: int, ultima_revisao: datetime
) -> date | None:
    if total_revisoes == 0:
        return None
    dias = INTERVALOS[total_revisoes] if total_revisoes < len(INTERVALOS) else 30
    return ultima_revisao.date() + timedelta(days=dias)
...
def obter_estudar_hoje(db: Session, usuario: User) -> dict:
    ...
    for topico in topicos:
        ...
        revisado_hoje = any(r.criado_em.date() == hoje for r in revisoes)
        if revisado_hoje:
            concluidos_hoje += 1

        proxima = calcular_proxima_revisao(total, ultima)
